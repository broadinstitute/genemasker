...
def main(args=None):
	...
	with ProgressBar():
		...
		def calculate_damaging_scores(logger, logger_handler):
			...
			chunk_paths_scored = fxns.merge_annot_scores(chunk_paths_orig, uid)
			return chunk_paths_scored
			
		@resource_tracker(logger)
		def calculate_filters(logger, logger_handler, chunk_paths_scored):
			
			logger.info("Calculating gene mask filters")
			chunk_paths_filters = fxns.calculate_mask_filters(chunk_paths_scored, run_masks, save_all = args.save_all)
			return chunk_paths_filters
			
		@resource_tracker(logger)
		def generate_group_files(logger, logger_handler, chunk_paths_filters):
		...
		if not args.generate_from_filtered:
			if not args.generate_from_scored:
				chunk_paths_scored = calculate_damaging_scores(logger, logger_handler)
			else:
				if args.chunk is not None:
					chunk_paths_scored = [args.generate_from_scored.replace("*",str(args.chunk))]
				else:
					chunk_paths_scored = glob.glob(args.generate_from_scored)
			chunk_paths_filters = calculate_filters(logger, logger_handler, chunk_paths_scored)
		else:
			if args.chunk is not None:
				chunk_paths_filters = [args.generate_from_filtered.replace("*",str(args.chunk))]
			else:
				chunk_paths_filters = glob.glob(args.generate_from_filtered)

		if args.chunk is None:
			generate_group_files(logger, logger_handler, chunk_paths_filters)
